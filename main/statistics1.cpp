                                            ...
                                            const std::string& result_filename)
{
	...
	while (std::getline(file, line))
	{
        oddsVector[i++] = boost::lexical_cast<double>(line);
	}
	file.close();
    ...
    for (auto state : cluster_map)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            for (int j = i + 1; j < size; j++)
            {
                double probability = oddsVector[state.second[i].BitMapMake(state.second[j])];
                if(probability == -1.0)
                {
                    probability = state.second[i].ProbabilityCalculate(state.second[j]);
                }

                #pragma omp critical
                output.push_back(std::string(std::to_string(state.second[i].id) + ',' + std::to_string(state.second[j].id) + ',' + std::to_string(probability)));
            }
            if (output.size() > 90000)
            {
                std::ofstream file;
